...
int polyglot_piece_types[COLOR_MAX][PIECE_TYPES] = 
{
    // black
  {
    0, 2, 4, 6, 8, 10
...
int polyglot_castle_offsets[COLOR_MAX][CASTLESIDE_MAX] = 
{
    // black
    {
        3, // queenside
...
uint64_t create_zobrist_from_scratch(Game * game){
    ...
    bool castle_set = false;

    // black
    for (int i = 0; i < PIECE_TYPES; i++){
        
        int piece_offset = 0;
        Piece p = piece_type_and_color_to_piece(i, BLACK);
        int kind_of_piece =  p - 1;
        
        uint64_t board = game->pieces[BLACK][i];
        while(board){
            int pos = pop_lsb(&board);
            File file;
            Rank rank;
            index_to_file_and_rank(pos, &file, &rank);
            pieces ^= Random64[(64 * kind_of_piece) + (8 * rank) + file];
        }
    ...
    for (int i = 0; i < PIECE_TYPES; i++){
        ...
        int piece_offset = 0;
        Piece p = piece_type_and_color_to_piece(i, WHITE);
        int kind_of_piece =  p + 1;

        uint64_t board = game->pieces[WHITE][i];
        while(board){
            int pos = pop_lsb(&board);
            File file;
            Rank rank;
    ...
    uint64_t side_to_move = 0;
    if (game->side_to_move == WHITE){
        side_to_move = Random64[TURN_OFFSET];
    }
    key = pieces ^ castle ^ en_passant ^ side_to_move;
...
uint64_t create_pawn_hash_from_scratch(Game * game){
    ...
    // } else if (w_king_pos & castle_masks[WHITE][QUEENSIDE]){
    //     key ^= king_location_random[WHITE][QUEENSIDE];
    // } else {
    //     key ^= king_location_random[WHITE][BOTHSIDE];
    // }
    // int b_king_pos = bit_scan_forward(&game->pieces[BLACK][KING]);
    // if (b_king_pos & castle_masks[BLACK][KINGSIDE]){
    //     key ^= king_location_random[BLACK][KINGSIDE];
    // } else if (b_king_pos & castle_masks[BLACK][QUEENSIDE]){
    //     key ^= king_location_random[BLACK][QUEENSIDE];
    // } else {
    //     key ^= king_location_random[BLACK][BOTHSIDE];
    // }
    return key;
...
void init_opening_book(Game * game, const char * path){
    ...
    for (int i = 0; i < entries; i++){
        ...
        create_new_polyglot_entry(game, key, weight, move);
    }
    fclose(b);
}
